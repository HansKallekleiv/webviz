...
class SumoCaseAccess:
    ...
    async def get_case_context(self) -> SearchContext:
        ...
        Raises:
            RuntimeError: If unable to create or retrieve case context
        """
        if not self._case_context:
            try:
                sc = SearchContext(sumo=self._sumo_client)
                self._case_context = await sc.get_case_by_uuid_async(self._case_uuid)
