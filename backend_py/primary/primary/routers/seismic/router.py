...
@router.get("/get_seismic_attribute_along_surface/")
async def get_seismic_attribute_along_surface(
    authenticated_user: AuthenticatedUser = Depends(AuthHelper.get_authenticated_user),
    case_uuid: str = Query(description="Sumo case uuid"),
    ensemble_name: str = Query(description="Ensemble name"),
    realization_num: int = Query(description="Realization number"),
    ...
    xtgeo_depth_surf = await surface_access.get_realization_surface_data_async(
        real_num=realization_num, name=surface_name, attribute=surface_attribute
    )
    import numpy as np

    print(
        xtgeo_depth_surf,
        np.min(xtgeo_depth_surf.values),
        np.max(xtgeo_depth_surf.values),
        xtgeo_depth_surf.xori,
        xtgeo_depth_surf.yori,
        xtgeo_depth_surf.xinc,
        xtgeo_depth_surf.yinc,
        xtgeo_depth_surf.rotation,
        xtgeo_depth_surf.fill(),
    )
    xtgeo_attr_surf = await vds_access.get_attribute_along_surface_as_xtgeo_surface(
        xtgeo_surface=xtgeo_depth_surf, above=1, below=1
    )
    print("*************************", xtgeo_attr_surf)
    meta: VdsMetadata = await vds_access.get_metadata_async()
    if len(meta.axis) != 3:
        raise ValueError(f"Expected 3 axes, got {len(meta.axis)}")
    depth_axis_meta = meta.axis[2]
