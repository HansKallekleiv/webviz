...
async def test_is_sensitivity_run_ahm(test_user, sumo_test_ensemble_ahm) -> None:
    assert (
        await router.is_sensitivity_run(
...
async def test_is_sensitivity_run_design(test_user, sumo_test_ensemble_design) -> None:
    assert (
        await router.is_sensitivity_run(
            test_user, sumo_test_ensemble_design.case_uuid, sumo_test_ensemble_design.ensemble_name
        )
        == True
    )
...
async def test_get_sensitivities_ahm(test_user, sumo_test_ensemble_ahm) -> None:
    sensitivities = await router.get_sensitivities(
        test_user, sumo_test_ensemble_ahm.case_uuid, sumo_test_ensemble_ahm.ensemble_name
    )
