import { SurfaceIntersectionData_api, WellBorePicksAndStratUnits_api, WellBoreTrajectory_api } from "@api";
import {
    Annotation,
    Axis,
    CalloutCanvasLayer,
...
export class ControllerHandler {
    ...
    public addSeismicLayer(
        ...
        // addSeismicOverlay(this.controller, dataValues);
    }

    public addGridParameterLayer(
        curtain: number[][],
        extension: number,
        image: ImageBitmap,
        dataValues: number[][],
        ...
        const layer = new SeismicCanvasLayer("grid-parameter", {
            order: 2,
            layerOpacity: 1,
        });
        layer.data = { image: image, options: getSeismicOptions(info) };
    ...
    public addSurfaceLayers(surfacIntersectionData: SurfaceIntersectionData_api[], pixiContext: PixiRenderApplication) {
        const geolayerdata = {
            areas: [],
            lines: surfacIntersectionData.map((surface) => {
                return {
                    data: surface.z_arr.map((z: number, idx) => {
                        return [surface.hlen_arr[idx], z];
                    }),
                    color: "black",
                    id: surface.name,
                    label: surface.name,
...
export function useGridParameterImageQuery(
    dataValues: number[][] | null,
    yValues: number[] | null,
    curtain: number[][] | null,
    colorscale: string[] | null,
    ...
    return useQuery({
        queryKey: ["gridParameterImage", dataValues, yValues, curtain, colorscale, colorMin, colorMax],
        queryFn: () =>
            generateGridSliceImage(
                { datapoints: dataValues ?? [], yAxisValues: yValues ?? [] },
                curtain ?? [],
                colorscale ?? [],
                {
                    isLeftToRight: true,
                    seismicMin: colorMin ?? undefined,
                    seismicMax: colorMax ?? undefined,
                }
            ),
        enabled: dataValues && yValues && curtain && colorscale ? true : false,
    });
