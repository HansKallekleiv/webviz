...
export function Settings(props: ModuleSettingsProps<State>) {
    const ensembleSet = useEnsembleSet(props.workbenchSession);
    const statusWriter = useSettingsStatusWriter(props.settingsContext);
    const colorSet = props.workbenchSettings.useColorSet();

    const [ensembleIdent, setEnsembleIdent] = props.settingsContext.useStoreState("ensembleIdent");
    const [realizations, setRealizations] = props.settingsContext.useStoreState("realizations");
    const [wellboreHeader, setWellboreHeader] = props.settingsContext.useStoreState("wellboreHeader");
    const [surfaceAttribute, setSurfaceAttribute] = props.settingsContext.useStoreState("surfaceAttribute");
    const [surfaceNames, setSurfaceNames] = props.settingsContext.useStoreState("surfaceNames");
    const [seismicDataType, setSeismicDataType] = props.settingsContext.useStoreState("seismicDataType");
    const [seismicTimeType, setSeismicTimeType] = props.settingsContext.useStoreState("seismicTimeType");
    const [seismicAttribute, setSeismicAttribute] = props.settingsContext.useStoreState("seismicAttribute");
    const [seismicTimestamp, setSeismicTimestamp] = props.settingsContext.useStoreState("seismicTimestamp");
    const [visibleLayers, setVisibleLayers] = props.settingsContext.useStoreState("visibleLayers");
    const [visibleStatisticCurves, setVisibleStatisticCurves] =
        props.settingsContext.useStoreState("visibleStatisticCurves");
    ...
    React.useEffect(function propogateColorsToView() {
        if (surfaceDirectory && availableSurfaceNames) {
            ...
            props.settingsContext.getStateStore().setValue("stratigraphyColorMap", surfaceColorMap);
        }
    });
