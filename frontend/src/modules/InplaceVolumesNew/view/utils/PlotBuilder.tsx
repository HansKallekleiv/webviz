...
export class PlotBuilder {
    ...
    private buildSubplots(
        ...
        const addedLegendEntries = new Set<string>();
        ...
        for (let row = 1; row <= numRows; row++) {
            for (let col = 1; col <= numCols; col++) {
                ...
                for (const plotData of plotDataArr) {
                    // Use the trace name as the unique identifier for legend entries
                    const legendKey = plotData.name?.toString() ?? "";

                    if (addedLegendEntries.has(legendKey)) {
                        plotData.showlegend = false;
                    } else {
                        addedLegendEntries.add(legendKey);
                    }
