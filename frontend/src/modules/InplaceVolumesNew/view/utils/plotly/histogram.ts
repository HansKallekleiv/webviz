...
function createStatisticLinesForHistogram(
    ...
    function createLine(value: number, label: string, dash: Dash): Partial<PlotData> {
        const trace: Partial<PlotData> = {
            ...
            name: label,
            legendgroup: title,
            hovertemplate: `<b>${title}</b><br><b>${label}</b><br>${resultName}: ${value ? formatValueWithAdaptivePrecision(value) : ""}<extra></extra>`,
            hoverlabel: { bgcolor: "white", font: { size: 12, color: "black" } },
        };

        if (showLabels) {
            trace.text = ["", `${label}: ${formatValueWithAdaptivePrecision(value)}`];
