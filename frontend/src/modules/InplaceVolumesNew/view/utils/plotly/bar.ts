...
export function makePlotlyBarTraces({
    ...
    const hoverText = sortedPoints.map(
        (p) =>
            `<b>${selectorName}:</b> ${p.x}<br><b>${resultName}:</b> ${formatValueWithAdaptivePrecision(Number(p.y))}<extra></extra>`,
    );
    ...
    data.push({
        ...
        text: showText ? sortedYValues.map((v) => formatValueWithAdaptivePrecision(v)) : undefined,
...
function createStatisticLinesForBarPlot(
    ...
    function createLine(value: number, label: string, dash: Dash): Partial<PlotData> {
        return {
            ...
            name: label,
            legendgroup: title,
            hovertemplate: `<b>${title}</b><br><b>${label}</b><br>${resultName}: ${formatValueWithAdaptivePrecision(value)}<extra></extra>`,
