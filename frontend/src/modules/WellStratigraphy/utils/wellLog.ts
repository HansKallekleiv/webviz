...
export function wellLogsToJson(wellName: string, tvd_arr: number[], md_arr: number[]): any {
    const header = generateHeader(wellName);
    const curves: Curve[] = [];

    curves.push(generateCurve("TVD", "True vertical depth (SS)"));
    curves.push(generateCurve("MD", "Measured depth"));
    const data = tvd_arr.map((tvd, i) => [tvd, md_arr[i]]);
...
export function createTemplate(): any {
    const tracks: any[] = [];
    const template = {
        name: "all logs",
    ...
    const plot = { name: "TVD", type: "gradientfill" };
    const track = { width: 2, title: "TVD", plots: [plot] };
    template.tracks.push(track);
    const plotmd = { name: "MD", type: "gradientfill" };
    const trackmd = { width: 2, title: "MD", plots: [plotmd] };
    template.tracks.push(trackmd);
