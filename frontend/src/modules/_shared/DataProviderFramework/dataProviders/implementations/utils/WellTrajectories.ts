...
function filterAndValidateCompletionsForWell(
    ...
    return wellCompletions.filter(
        (comp) =>
            comp.topDepthMd != null &&
            comp.baseDepthMd != null &&
            comp.topDepthMd <= comp.baseDepthMd &&
            requestedCompletionTypes.has(comp.completionType as CompletionType) &&
            isCompletionOverlappingDateInterval(comp, filterStartMs, filterEndMs) &&
...
/**
 * Add segmentation of a single well's trajectory using interpolation.
 */
function processSingleWellForSegments_Interpolated(
    ...
    const numSurveyPoints = wellData.mdArr?.length ?? 0;
    ...
    const sortedBreakpoints = collectAndSortBreakpoints(wellData, relevantCompletions, stratIntervals);
    ...
    for (let i = 0; i < sortedBreakpoints.length - 1; i++) {
        ...
        if (stratIntervals && stratIntervals.length > 0) {
            ...
            // If NO strat intervals apply to this well
            // then DON'T filter based on stratigraphy
            isWithinStratUnit = true;
        }
        ...
        // --- Do only if within strat unit ---

        // 4. Get completion type for the interval
...
export function filterDataByFlow(
    ...
    for (const flowData of flowDataArr) {
        const identifier = flowData.well_uwi;
        if (identifier) {
...
export function segmentDataByCompletionAndStratigraphy(
